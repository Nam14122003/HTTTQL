...
const InventoryList = () => {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/products/export-excel', {
        params,
        responseType: 'blob',
        headers: {
      ...
      await saveFile(blob, 'products_export.xlsx');
      setError(null);
    } catch (err) {
  ...
  const handleImportExcel = async (e) => {
    ...
    setImporting(false);
    // Reset input value để có thể chọn lại cùng 1 file nếu cần
    e.target.value = '';
  };
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    {products.map(product => (
                        <tr key={product.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{product.size} / {product.color}</td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {product.status === 'available' && <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Còn hàng</span>}
                            {product.status === 'out_of_stock' && <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Hết hàng</span>}
                            {product.status === 'discontinued' && <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Ngừng kinh doanh</span>}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <Link to={`/inventory/edit/${product.id}`} className="text-blue-600 hover:text-blue-900 mr-4">Chỉnh sửa</Link>
                            <button onClick={() => confirmDelete(product)} className="text-red-600 hover:text-red-900">Xóa</button>
                          </td>
                        </tr>
                    ))}
                    </tbody>
                  ...
                  <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div className="sm:flex sm:items-start">
                      ...
                      <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 className="text-lg leading-6 font-medium text-gray-900" id="modal-title">Xác nhận xóa sản phẩm</h3>
                        <div className="mt-2">
                          <p className="text-sm text-gray-500">Bạn có chắc chắn muốn xóa sản phẩm "{productToDelete?.name}"? Hành động này không thể hoàn tác.</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="button" className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm" onClick={handleDelete}>Xóa</button>
                    <button type="button" className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" onClick={() => {setShowDeleteModal(false); setProductToDelete(null);}}>Hủy</button>
                  </div>
                </div>
              </div>
            </div>
